package com.companyname.casusbelli.core.logging;
...
//    	log.info("Web element " + lastFindBy + " is found by web driver");
    }
    ...
    public void onException(Throwable stacktrace, WebDriver selenium){
        if (stacktrace.getClass().equals(NoSuchElementException.class)){
            log.error("WebDriver error: Element not found "+lastFindBy);
        } else {
            log.error("WebDriver error: ", stacktrace);
        }
    }
}